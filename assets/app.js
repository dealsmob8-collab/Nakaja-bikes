...
function buildCashWhatsappMessage(product) {
  return appendPromoLine(
    [
      ...
      `Cash price shown is ${formatKes(product.cashPrice)}.`,
  ...
  try {
    ...
    const renderOutput = () => {
      ...
      output.innerHTML = `
        ...
        <p><strong>Cash price:</strong> ${escapeHtml(formatKes(product.cashPrice))}</p>
...
function buildProductSummaryCards(product, fortunePlans, watuPlans) {
  const cards = [
    buildSummaryCard("Cash Price", formatKes(product.cashPrice), "Strongest public price for this model."),
    fortunePlans.length
      ? buildSummaryCard(
          "Fortune Start With",
...
function buildProductDetails(product, plans) {
  const details = [
    ...
    ["Cash price", formatKes(product.cashPrice)],
